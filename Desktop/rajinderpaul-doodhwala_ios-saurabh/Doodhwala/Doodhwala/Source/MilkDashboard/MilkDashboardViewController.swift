...
class DoodhvaleCustomSlideView: ImageSlideshow {
    ...
    override func layoutPageControl() {
        ...
        pageControl.frame = CGRect(x: frame.size.width - 70, y: frame.size.height - 25.0, width: 40, height: 20)
...
class MilkDashboardViewController: InsufficientViewController {
    ...
    fileprivate func setUpSlideShowView(imagesArray: [SDWebImageSource]) {
        ...
        if imagesArray.count > 0 {
            ...
            slideShowImageView.setImageInputs([ImageSource(image: UIImage(named: "Banner2")!),
    ...
    fileprivate func callDeliveryBoy(number: String)  {
        ...
        if let url = URL(string: "TEL://\(number)") {
            ...
            if (application.canOpenURL(url)) {
                let alert = UIAlertController(title: "", message:  "Do you want to call area manager?", preferredStyle: .alert)
                let callAction = UIAlertAction(title: "Make Call", style: .default, handler: { (action) in
...
extension MilkDashboardViewController {
    func fcmtokengetting() {
         
          if let userId = UserManager.defaultManager.userDict?["user_id"] as? Int {
           
                let appDelegate = UIApplication.shared.delegate as! AppDelegate
                 let notificationtoken = appDelegate.token
                  print(notificationtoken)
            APIManager.defaultManager.fcmtoken(parameters: ["user_id": userId,   "fcm_token_id" : "\(notificationtoken)" ]) { (responseDict) in
