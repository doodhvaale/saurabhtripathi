...
class AccountHistoryViewController: InsufficientViewController {
                    ...
                    if let status = responseDict["status"] as? String {
                        ...
                        if status == "000000" {
                            ...
                            if var amount = responseDict["pending_amount"] as? Double {
                                ...
                                self.walletBalanceLabel.text =   "₹" + String(format: "%.2f", amount)
                            }
                            if let amount = responseDict["bill_amount"] as? Double {
                               // self.pendingAmountLabel.text = "₹ " + "\(amount)"
                               self.pendingAmountLabel.text =   "₹" + String(format: "%.2f", amount)
                            }
